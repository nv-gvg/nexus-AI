...
class MessageWidget(QFrame):
    ...
    def __init__(self, message_id: str, role: str, content: str, created_at: str, parent=None) -> None:
        ...
        role_label.setStyleSheet(
            f"font-weight: bold; color: {theme.ACCENT if is_user else '#7dd0a0'};"
        )
        header.addWidget(role_label)
        header.addStretch(1)
        time_label = QLabel(created_at)
        time_label.setStyleSheet(f"color: {theme.TEXT_DIM}; font-size: 11px;")
        ...
        self.content_label.setStyleSheet(f"color: {content_color}; font-size: 14px;")
