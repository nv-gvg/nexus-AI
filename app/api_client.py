...
class APIClient:
    ...
    def _build_messages(
        self, messages: list[dict[str, str]], system_extra: str = ""
    ) -> list[dict[str, str]]:
        system_prompt = self.config.get("system_prompt", "")
        if system_extra:
            system_prompt = f"{system_prompt}\n\n{system_extra}"
        return truncate_history(messages, system_prompt)
    ...
    def stream_chat(
        ...
        若传入 mcp_client 且存在可用工具，先执行工具调用循环（非流式），
        最终答案通过 on_token 一次性返回。
        """
        if mcp_client is not None and getattr(mcp_client, "enabled", False):
            tools = mcp_client.list_tools()
            if tools:
                return self._run_tool_loop(
                    messages, system_extra, tools, mcp_client, on_token, should_stop
                )
    ...
    def _post_once(self, api_key: str, payload: dict[str, Any]) -> dict[str, Any]:
        """非流式请求，返回完整 JSON。"""
        url = self._url()
        try:
            resp = requests.post(
                url, headers=self._headers(api_key), json=payload, timeout=(10, 180)
            )
